 ...
 */
export declare function parseHeader(buf: Buffer, dcidLen: number): NapiResult
/**
 ...
 */
export declare function negotiateVersion(scid: Buffer, dcid: Buffer, out: Buffer): NapiResult
/**
 ...
 * @returns Number of bytes written
 */
export declare function retry(scid: Buffer, dcid: Buffer, newScid: Buffer, token: Buffer, version: number, out: Buffer): NapiResult
/**
...
export const enum StartupExitReason {
 ...
 */
export declare function generateCid(len: number): NapiResult
/**
 ...
 */
export declare function encodeAlpn(protocols: Array<string>): NapiResult
/**
...
export declare class Config {
   ...
   */
  loadCertChainFromPemFile(path: string): NapiResult
  /**
   * Load private key from PEM file
   *
   * @param path - Path to PEM file containing private key
   */
  loadPrivKeyFromPemFile(path: string): NapiResult
  /**
   ...
   */
  setApplicationProtos(protos: Buffer): NapiResult
  /**
...
export declare class Connection {
   ...
   */
  static connect(scid: Buffer, local: string, peer: string, config: Config): NapiResult
  /**
   ...
   * @param config - QUIC configuration
   */
  static accept(scid: Buffer, odcid: Buffer | undefined | null, local: string, peer: string, config: Config): NapiResult
  /**
   ...
   */
  recv(buf: Buffer, from: string): NapiResult
  /**
   ...
   * @returns Number of bytes written
   */
  streamSend(streamId: number, data: Buffer, fin: boolean): NapiResult
  /**
   ...
   */
  streamRecv(streamId: number, out: Buffer): NapiResult
   ...
   */
  close(app: boolean, errCode: number, reason: Buffer): NapiResult
  /**
   ...
   */
  setQlog(path: string, title: string, description: string): NapiResult
   ...
   */
  streamPriority(streamId: number, urgency: number, incremental: boolean): NapiResult
  /**
   ...
   */
  isPathValidated(local: string, peer: string): NapiResult
   ...
   */
  probePath(local: string, peer: string): NapiResult
  /**
   ...
   */
  migrate(local: string, peer: string): NapiResult
  /**
   ...
   * @returns DCID sequence number for the new path
   */
  migrateSource(local: string): NapiResult
  /**
   ...
   */
  retireDcid(dcidSeq: number): NapiResult
  /**
   ...
   */
  dgramRecv(buf: Buffer): NapiResult
  /**
   * Send a QUIC datagram.
   *
   * @param buf - Datagram data to send
   */
  dgramSend(buf: Buffer): NapiResult
  /**
...
export declare class H3Connection {
   ...
   */
  static withTransport(conn: Connection, config: H3Config): NapiResult
  /**
   * Send an HTTP/3 request
   *
   * Returns the stream ID on which the request was sent.
   */
  sendRequest(conn: Connection, headers: Array<Header>, fin: boolean): NapiResult
  /** Send an HTTP/3 response */
  sendResponse(conn: Connection, streamId: number, headers: Array<Header>, fin: boolean): NapiResult
  /**
   * Send body data on a stream
   *
   * Returns the number of bytes written.
   */
  sendBody(conn: Connection, streamId: number, body: Buffer, fin: boolean): NapiResult
  /**
   * Receive body data from a stream
   *
   * Returns the number of bytes read.
   */
  recvBody(conn: Connection, streamId: number, out: Buffer): NapiResult
  /**
   * Poll for HTTP/3 events
   *
   * Returns an event object or null if no events are available.
   */
  poll(conn: Connection): NapiResult
