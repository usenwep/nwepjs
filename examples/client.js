...
// Create QUIC config
const config = new __1.Config(__1.PROTOCOL_VERSION);
config.verifyPeer(false); // Skip cert verification for self-signed cert
config.setApplicationProtos((0, __1.nwepAlpn)());
...
const h3Config = new __1.H3Config();
console.log('connecting to web://' + SERVER_HOST + ':' + SERVER_PORT + PATH);
// Create UDP socket
const socket = dgram_1.default.createSocket('udp4');
// Create connection
const scid = (0, __1.generateCid)(16);
const conn = __1.Connection.connect(scid, '0.0.0.0:0', `${SERVER_HOST}:${SERVER_PORT}`, config);
let h3Conn = null;
...
socket.on('message', (msg, rinfo) => {
    ...
    if (conn.isEstablished() && !h3Conn) {
        ...
        try {
            h3Conn = __1.H3Connection.withTransport(conn, h3Config);
