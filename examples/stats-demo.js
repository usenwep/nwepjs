      ...
      try {
        ...
        if (result.fin) {
          ...
          try {
            ...
            // Print statistics after echoing
            if (!client.statsShown) {
              console.log('\n=== Connection Statistics ===');
              const stats = client.conn.stats();
              console.log('Packets recv:', stats.recv, 'sent:', stats.sent, 'lost:', stats.lost);
              console.log('Bytes recv:', stats.recvBytes, 'sent:', stats.sentBytes);

              console.log('\n=== Path Statistics ===');
              const pathStats = client.conn.pathStats();
              pathStats.forEach((path, idx) => {
                console.log(`Path ${idx}: ${path.localAddr} <-> ${path.peerAddr}`);
                console.log(`  State: ${path.validationState}, Active: ${path.active}`);
                console.log(`  RTT: ${path.rttMs.toFixed(2)}ms, CWND: ${path.cwnd}, PMTU: ${path.pmtu}`);
              });
              console.log('');
