...
let h3Conn: { isNwep: (arg0: any) => any; sendRequest: (arg0: any, arg1: { name: Buffer<ArrayBuffer>; value: Buffer<ArrayBuffer>; }[], arg2: boolean) => any; poll: (arg0: any) => any; recvBody: (arg0: any, arg1: any, arg2: Buffer<ArrayBuffer>) => any; } | null = null;
let requestSent = false;
let responseReceived = false;
const out = Buffer.allocUnsafe(MAX_DATAGRAM_SIZE);

socket.on('message', (msg: any, rinfo: { address: any; port: any; }) => {
  // Process incoming packet
  try {
    conn.recv(msg, `${rinfo.address}:${rinfo.port}`);
  } catch (e) {
    if (!((e as Error).message).includes('Done')) {
      console.error('recv error:', (e as Error).message);
    }
  }
  ...
  if (conn.isEstablished() && !h3Conn) {
    ...
    try {
      ...
      // Check if using NWEP
      if (h3Conn && h3Conn.isNwep(conn)) {
        console.log('using nwep protocol');
      }
    } catch (e) {
      console.error('h3 connection failed:', (e as Error).message);
    }
  }
  ...
  if (h3Conn && !requestSent) {
    ...
    try {
      ...
      console.error('send request error:', (e as Error).message);
    }
  }
  ...
  if (h3Conn) {
    while (true) {
      ...
      if (event.eventType === 'headers') {
        ...
        headers.forEach((h: { name: { toString: () => any; }; value: { toString: () => any; }; }) => {
      ...
      if (event.eventType === 'data') {
        ...
        try {
          ...
          if (!((e as Error).message).includes('Done')) {
            console.error('recv body error:', (e as Error).message);
          }
        }
      }
